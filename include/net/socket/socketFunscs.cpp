...
bool socketFuncs::Bind(SOCKET sockfd, std::string ip, uint16_t port)
{
...
void socketFuncs::Close(SOCKET sockfd)
{
#if defined(__linux) || defined(__linux__)
    ::close(sockfd);
#elif defined(WIN32) || defined(_WIN32)
...
bool socketFuncs::Connect(SOCKET sockfd, std::string ip, uint16_t port, int timeout)
{
    bool is_connected = true;

    if (timeout > 0)
    {
        socketFuncs::SetNonBlock(sockfd);
    }
